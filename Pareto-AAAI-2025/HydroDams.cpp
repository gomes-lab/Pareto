...
void run() {
    ...
    assert(net->num_criteria == config.upper_bounds.size() || config.upper_bounds.size() == 0);
    assert(net->num_criteria == config.lower_bounds.size() || config.lower_bounds.size() == 0);
    assert(config.optimized_criteria.size() <= net->num_criteria);

    std::cout << "Beginning DP" << std::endl;

    assert ((config.use_linear_preferences && (int)config.w.size() > 0) || (!config.use_linear_preferences && (int)config.w.size() == 0));
    ParetoDP dp(net, io, config, false);
